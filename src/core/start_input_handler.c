...
void handle_start_input(APP_CONTEXT *ctx)
{
    ...
    else if (ctx->active_window_idx >= 2 && ctx->active_window_idx <= 4)
    {
        switch (ctx->key)
        {
            case 9:
            case KEY_RIGHT:
                if (ctx->active_window_idx == 2 || ctx->active_window_idx == 3)
                {
                    ...
                    ctx->start_windows[ctx->active_window_idx - 1] =
                        create_course_preview_card(
                            ctx, CARD_WIDTH * (ctx->active_window_idx - 3),
                ...
                else
                {
                    ...
                    ctx->start_windows[ctx->active_window_idx - 1] =
                        create_course_preview_card(
                            ctx, CARD_WIDTH * (ctx->active_window_idx - 3),
                            ctx->active_window_idx - 1, &ctx->courses[2]);
                    focus_window(&ctx->start_windows[5], 3, "Details");
                }
                doupdate();
                break;
            case KEY_LEFT:
                if (ctx->active_window_idx > 2)
                {
                    ctx->active_window_idx--;
                    ctx->active_window =
                        ctx->start_windows[ctx->active_window_idx];
